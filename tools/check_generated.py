...
**この検査で捕まえられる範囲(はっきりさせておく):**
 ・データを変えたのに再生成していない → 捕まえる(実験で確認)
 ・生成物を手で書き換えて、生成器が上書きする領域に入っている → 捕まえる
 ・生成物を手で書き換えて、**マーカーの外側**に足した → **捕まえられない**
   (生成器はマーカーの間だけを差し替えるので、外側の追記は再生成しても残る)
   こちらは pre-commit 側で「データも道具も触らずに生成物だけをコミットしていないか」
   を見ることで押さえる。
